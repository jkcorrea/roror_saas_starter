...
class StripeWebhookService
  def no_account_error(klass, account_stripe_customer_id)
    Rails.logger.error "#{klass.class.name.upcase} ERROR: No account found with stripe_customer_id #{account_stripe_customer_id}."
    yield
  end
  ...
  def assign_plan(account_attributes, stripe_plan)
    ...
    if plan.present?
      ...
      Rails.logger.error "ASSIGN_PLAN ERROR: No local plan found for #{stripe_plan.id}"
    end
  end
  ...
  def process_subscription_status(subscription, account_attributes)
    ...
    when "unpaid"
      ...
      Rails.logger.error "#{self.class.name} ERROR: Unknown subscription status #{subscription.status}."
    end
    ...
    def call(event)
      ...
      if sources.present?
        if sources.total_count > 1
          Rails.logger.error "UpdateCustomer ERROR: Customer #{event_data.id} has #{sources.total_count} sources. There's supposed to have one!"
        ...
        Rails.logger.error "UpdateCustomer ERROR: Customer #{event_data.id} has no source."
      end
      ...
      if subscriptions.present?
        if subscriptions.total_count > 1
          Rails.logger.error "UpdateCustomer ERROR: Customer #{event_data.id} has #{subscriptions.total_count} subscriptions. They're supposed to have one!"
        ...
        Rails.logger.error "UpdateCustomer ERROR: Customer #{event_data.id} has no subscription."
      end
